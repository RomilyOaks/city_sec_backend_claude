/**
 * Ruta: src/routes/tipo-novedad.routes.js
 * VERSIÓN: 1.0.0
 */

import express from "express";
import subtipoNovedadController from "../controllers/subtipoNovedadController.js";
import {
  verificarToken,
  verificarRoles,
} from "../middlewares/authMiddleware.js";
import {
  validateCreate,
  validateUpdate,
  validateId,
  validateQuery,
} from "../validators/subtipo-novedad.validator.js";

const router = express.Router();

// Todas las rutas requieren autenticación
router.use(verificarToken);

/**
 * @route GET /api/v1/subtipos-novedad
 * @desc  Listar Subtipos de novedad
 * @access Private (todos autenticados)
 */
router.get("/", validateQuery, subtipoNovedadController.getAll);

/**
 * @route GET /api/v1/subtipos-novedad/:id
 * @desc  Obtener tipo de novedad por ID
 * @access Private (todos autenticados)
 */
router.get("/:id", validateId, subtipoNovedadController.getById);

/**
 * @route POST /api/v1/subtipos-novedad
 * @desc  Crear tipo de novedad
 * @access Private (admin, supervisor)
 */
router.post(
  "/",
  verificarRoles(["super_admin", "admin", "supervisor"]),
  validateCreate,
  subtipoNovedadController.create
);

/**
 * @route PUT /api/v1/subtipos-novedad/:id
 * @desc  Actualizar tipo de novedad
 * @access Private (admin, supervisor)
 */
router.put(
  "/:id",
  verificarRoles(["super_admin", "admin", "supervisor"]),
  validateUpdate,
  subtipoNovedadController.update
);

/**
 * @route DELETE /api/v1/subtipos-novedad/:id
 * @desc  Eliminar tipo de novedad
 * @access Private (admin)
 */
router.delete(
  "/:id",
  verificarRoles(["super_admin", "admin"]),
  validateId,
  subtipoNovedadController.remove
);

export default router;

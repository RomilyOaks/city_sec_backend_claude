/**
 * =========================================================
 * CONTROLADOR: src/controllers/auditoriaAccionController.js
 * =========================================================
 *
 ...
 * @route GET /api/AuditoriaAccion/:id
 */
export const getAuditoriaAccionById = async (req, res) => {
  try {
    const { id } = req.params;
 ...
 * @route GET /api/AuditoriaAccion/entidad/:entidad/:id
 */
export const getHistorialEntidad = async (req, res) => {
  try {
 ...
 * @route GET /api/AuditoriaAccion/stats
 */
    ...
    res.status(200).json({
      ...
      data: {
        ...
        tasaExito:
          totalAcciones > 0
            ? (
              ((totalAcciones - accionesFallidas) / totalAcciones) *
                100
            ).toFixed(2)
            : 0,
        accionesPorTipo,
 ...
 * @route GET /api/AuditoriaAccion/mi-actividad
 */
 ...
 * @route GET /api/AuditoriaAccion/export/csv
 */
export const exportarCSV = async (req, res) => {
  try {
    ...
    // Generar CSV
    let csv =
      "ID,Fecha,Usuario,Acción,Entidad,Descripción,Severidad,Resultado,IP\n";

    auditorias.forEach((audit) => {
      csv += `${audit.id},"${audit.created_at}","${
        audit.usuario?.username || "Sistema"
      }","${audit.accion}","${audit.entidad}","${audit.descripcion || ""}","${
        audit.severidad
      }","${audit.resultado}","${audit.ip_address || ""}"\n`;
    });

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=AuditoriaAccion_${Date.now()}.csv`
    );

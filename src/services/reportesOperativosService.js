  ...
  try {
    ...
    const estadisticasPrioridades = processedResults.reduce((acc, item) => {
      ...
      if (!acc[prioridad]) {
        acc[prioridad] = {
          count: 0,
          color: prioridad === "ALTA" ? "rojo" : prioridad === "MEDIA" ? "ambar" : prioridad === "BAJA" ? "verde" : "gris",
          novedades: []
        };
      }
      acc[prioridad].count++;
    ...
    const baseQuery = `
      SELECT
        ...
        CONCAT(ps_sis.nombres,
                ...
                ' ',
                ps_sis.apellido_materno) AS Usuario_Operador_Sistema,
        carg_sis.nombre Cargo_Usuario_Operador,
        ...
        op.updated_by as ID_Usuario_Actualizador_Patrullaje_Pie,
        ...
        opn.updated_by AS ID_Usuario_Actualizador_Novedad,
...
export const getNovedadesNoAtendidas = async (queryParams = {}) => {
  try {
    ...
    // Procesar resultados para convertir campos numéricos
    const processedResults = novedades.map(item => {
      // Convertir perdidas_materiales_estimadas a número
      if (item.perdidas_materiales_estimadas !== null && item.perdidas_materiales_estimadas !== undefined) {
        item.perdidas_materiales_estimadas = parseFloat(item.perdidas_materiales_estimadas) || 0;
      }
      return item;
    });
